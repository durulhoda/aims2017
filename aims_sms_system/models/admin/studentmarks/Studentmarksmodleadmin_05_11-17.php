...
//        $this->db->order_by("id", "asc");
    ...
    // $sql="SELECT semesterId,examtypeId,programOfferId,studentId,SUM(marks) as marks FROM studentmarks where `programOfferId`='".$data['programOfferId']."' AND `semesterId`='".$data['semesterId']."' AND `examtypeId`='".$data['examtypeId']."' GROUP BY studentId order by marks DESC";
      $sql="SELECT studentmarks.semesterId, studentmarks.examtypeId, studentmarks.programOfferId,studentmarks.studentId,programoffer.programLevel, SUM(studentmarks.marks) as marks FROM studentmarks JOIN programoffer ON studentmarks.programOfferId = programoffer.programOfferId where studentmarks.programOfferId='".$data['programOfferId']."' AND studentmarks.semesterId='".$data['semesterId']."' AND studentmarks.examtypeId='".$data['examtypeId']."' GROUP BY studentId ORDER BY marks DESC";
