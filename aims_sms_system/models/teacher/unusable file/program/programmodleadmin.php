...
class ProgramModleAdmin extends CI_Model {
    ...
    public function getlistProgram(){
        $this->db->order_by('programLevel','ASC'); 
...
//        $this->db->select('campusName');        
    ...
    public function duplicateProgramOffer($data) {       
        $quu = $this->db->get_where($this->_offertable, array('programLevel' => $data['programLevel'],'programId' => $data['programId'], 'mediumId' => $data['mediumId'],'groupId'=> $data['groupId'],'shiftId' => $data['shiftId'],'sessionId' => $data['sessionId'],'employeeId'=>$data['employeeId'],'applicantSeat'=>$data['applicantSeat'],'classStatus' => $data['classStatus']));
