...
<div class="row">
    ...
    ?>
    <div class="col-xs-12">
        <form class="form-horizontal" role="form" action="<?php echo admin_Url(); ?>/studentmarks/updateStudentmarks/<?php echo $editData['markId'] ?>" enctype="multipart/form-data" method="post">
                                    ...
                                    if(!empty($dividemark)){
                                        ...
                                        for($mrk=0; $mrk<$count_markval; $mrk++){
                                          $mark_val[$mrk];
                                           ...
                                           ?>
                                           <div class="has-error col-xs-10 col-sm-4">
                                                <label class="control-label" for="form-field-1"><?php if(!empty($mrkttl)){ echo $mrkttl;} ?>  &nbsp; <?php echo form_error('data[marks]', '<span class="successMessage">', '</span>'); ?></label>
                                                <td class="hidden-480">
                                                    <input type="text" name="other_marks[]" placeholder="<?php echo $mrkttl."-".$mark_val[$mrk]; ?>" value="<?php if(!empty($obtain_mark[$mrk])){ echo $obtain_mark[$mrk];} ?>" class="form-control" id="form-field-1">                                
                                                </td>
                                            </div>
