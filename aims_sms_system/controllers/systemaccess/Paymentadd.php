...
//        print_r($data);exit;
        ...
        if (!empty($data['studentId']) && $data['sessionId']) {
            ...
            $countdata=count($data['test']);
             //echo "<pre>";print_r($countdata);die();
                   //    echo ($countdata); die();
            ...
            if (!empty($data['info'])) { 
              ...
              // echo "<pre>";print_r ($data['finecauselist']);die();
                if ($countdata==1) {
    ...
    public function insertpaymentadd() {
        ...
        if (!empty($_POST['headId'])) {
        	//echo "<pre>";print_r($_POST['headId']);die();
           // echo '<pre>';print_r($_POST);exit;
      ...
      //  print_r($data); die();
            ...
            $data['mr_no'] = $this->getMrNo($programOfferId, $studentId);
...
//                     redirect(admin_Url() . "/paymentadd");
    ...
    public function getMrNo($programOfferId = 0, $studentId = 0)
    {
        $mr_no = 0;
        $record = $this->db
                    ->select('mr_no')
